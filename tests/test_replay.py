...
def test_bridge_debug_pixel_reports_a_full_list_still_truncated_at_the_ceiling(
    monkeypatch, tmp_path
):
    """Asking for exactly the ceiling must not hide that stepping was cut short."""
    controller = FakeController()
    endless = [_debug_state(step) for step in range(MAX_DEBUG_STEPS)]
    ...
    status, _ = run_bridge(
        ...
        tmp_path,
        "debug_pixel",
        {"x": 1, "y": 2, "max_steps": MAX_DEBUG_STEPS},
        controller,
    )
    result = status["result"]
    assert result["step_count"] == MAX_DEBUG_STEPS
    assert len(result["steps"]) == MAX_DEBUG_STEPS
    # The list is as long as requested, but completion was never observed.
    assert result["truncated"] is True
