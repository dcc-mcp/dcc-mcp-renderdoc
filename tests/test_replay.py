...
from dcc_mcp_renderdoc import replay, runtime
...
class FakePipeState:
    ...
    def GetShaderReflection(self, stage):
        ...
        return SimpleNamespace(
            ...
            outputSignature=[],
            constantBlocks=[
                SimpleNamespace(name="cb0", fixedBindNumber=0, byteSize=64, bufferBacked=True)
            ],
            readOnlyResources=[
                SimpleNamespace(
                    name="albedo",
...
def test_bridge_get_texture_data_saves_through_renderdoc(monkeypatch, tmp_path):
    ...
    result = status["result"]
    assert result == {"resource_id": 11, "output_file": str(output), "size_bytes": 3}
...
def test_replay_skill_declares_one_tool_per_script():
    root = Path(replay.__file__).parent / "skills" / "renderdoc-replay"
