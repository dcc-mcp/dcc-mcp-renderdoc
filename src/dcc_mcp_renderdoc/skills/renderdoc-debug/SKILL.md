...
- when given `capture_file`, RenderDoc's own per-capture replay flags — `pixel_history`,
  `shader_debugging`, `post_vs_data` — which vary per capture and per driver;
...
| Tool | Needs | Answers |
| --- | --- | --- |
| `pick_pixel` | deep backend | which draw last wrote this pixel, and what it wrote |
...
3. `pick_pixel` — confirm the draw that wrote the pixel, and read its value.
